...
As such, once the slot pointer points to the root of a struct, we can view contiguous items by simply incrementing the slot pointer. The [Builder API](/architecture/builder-api-methods) exposes methods to do this (`addSlot`).
